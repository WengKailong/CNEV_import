...
const cors = corsFactory({
  origin: (origin, cb) => {
    ...
    const allowed: string[] = originStr
      .split(',')
      .map((s: string) => s.trim())
      .filter(Boolean);
...
async function verifyRecaptcha(token: string): Promise<void> {
  ...
  type RecaptchaResp = { success?: boolean; score?: number;[k: string]: unknown };
  const data = (await resp.json()) as RecaptchaResp;
...
export const submitLead = onRequest(async (req: any, res: any) => {
